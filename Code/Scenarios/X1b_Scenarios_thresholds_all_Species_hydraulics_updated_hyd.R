...
probs_function <- function(Model) {
  if(Model == "willow_StreamPower_adult") {
    probs <- expression(pnorm(willow_StreamPower_adultx, mean=mean(log(StreamPower+1)),
                              sd=sd(log(StreamPower+1))))
    
  } else if(Model == "Cladophora_Shear_adult" ) {
    probs <- expression(pnorm(Cladophora_Shear_adultx, mean=mean(log(shear+1)),
                              sd=sd(log(shear+1))))
    
  } else if(Model == "Steelhead_Depth_Prolonged") {
    probs <-  expression(pnorm(Steelhead_Depth_Prolongedx, mean=mean(log(depth_cm+1)),
                               sd=sd(log(depth_cm+1)), lower.tail = FALSE)) 
    
  } else if(Model == "Steelhead_Velocity_Prolonged") {
    probs <- expression(pnorm(Steelhead_Velocity_Prolongedx, mean=mean(log(Velocity+1)),
                              sd=sd(log(Velocity+1))))
    
  } else if(Model == "Steelhead_Depth_Burst") {
    probs <- expression(pnorm(Steelhead_Depth_Burstx, mean=mean(log(depth_cm+1)),
                              sd=sd(log(depth_cm+1)), lower.tail = FALSE))
    
  } else if (Model == "Steelhead_Velocity_Burst") {
    probs <- expression(pnorm(Steelhead_Velocity_Burstx, mean=mean(log(Velocity+1)),
                              sd=sd(log(Velocity+1))))
    
  } else if(Model == "Steelhead_Depth_smolts") {
    probs <- expression(pnorm(Steelhead_Depth_smoltsx, mean=mean(log(depth_cm+1)),
                              sd=sd(log(depth_cm+1)), lower.tail = FALSE)) 
    
  } else if(Model == "min_depth_limit") {
    probs <- expression(pnorm(min_depth_limitx, mean=mean(log(depth_cm+1)),
                              sd=sd(log(depth_cm+1)), lower.tail = FALSE)) 
  }
  return(probs)
...
willow_StreamPower_adultx <- log(4000+1)
Cladophora_Shear_adultx <- log(16.9+1)
Steelhead_Depth_Prolongedx <- log(23+1)
Steelhead_Velocity_Prolongedx <- log(2+1)
Steelhead_Depth_Burstx <- log(18+1)
Steelhead_Velocity_Burstx <- log(3.1+1)
Steelhead_Depth_smoltsx <- log(12+1)

min_depth_limitx <- log(3+1)
h
n=3
...
for(n in 1: length(h)) {
  ...
  for(s in 1:length(scenarios)) {
    ...
    for(p in 1:length(positions)) {
      ...
      # testdry <- thresholds_datax %>% filter(season == "Dry")
      # testwet <- thresholds_datax %>% filter(season == "Wet")
      # 
      # 
      #   range(testdry$depth_cm)
      #   range(testwet$depth_cm)
      #   
      #   sum(testdry$depth_cm >= 23)/length(testdry$depth_cm)*100
      #   sum(testwet$depth_cm >= 23)/length(testwet$depth_cm)*100
      #   Steelhead_Depth_Prolongedx
      #   
      #   pnorm(log(Steelhead_Depth_Prolongedx), mean = mean(log(testdry$depth_cm)), 
      #         sd = sd(log(testdry$depth_cm)), lower.tail = FALSE)
      #   
      #   pnorm(log(Steelhead_Depth_Prolongedx), mean = mean(log(testwet$depth_cm)), 
      #         sd = sd(log(testwet$depth_cm)), lower.tail = FALSE)
      #   
      #   pnorm(Steelhead_Depth_Prolongedx, mean = mean(testdry$depth_cm), 
      #         sd = sd(testdry$depth_cm), lower.tail = FALSE)
      #   
      #   pnorm(Steelhead_Depth_Prolongedx, mean = mean(testwet$depth_cm), 
      #         sd = sd(testwet$depth_cm), lower.tail = FALSE)
      #   
      #   hist(log(testwet$depth_cm))
        # hist(log(testdry$depth_cm))
        ...
        # Steelhead_Depth_Prolongedx
        # 
        # pnorm(Steelhead_Depth_Prolongedx, mean = mean(log(thresholds_datax$depth_cm+1)), 
        #       sd = sd(log(thresholds_datax$depth_cm+1)), lower.tail = FALSE)
