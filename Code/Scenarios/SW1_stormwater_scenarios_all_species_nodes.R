...
for(n in 1: length(h)) {
  ...
  for(c in 1: length(QC)) {
    ...
    ## save df
    write.csv(melt_timex, paste("output_data/scenarios/stormwater/SW1", NodeName, SpeciesName, LifeStageName, HydraulicName,  "time_stats_updated_hyd_scenarios_stormwater.csv", sep="_"))
    
    write.csv(melt_days, paste("output_data/scenarios/stormwater/SW1", NodeName, SpeciesName, LifeStageName, HydraulicName,  "total_days_long_updated_hyd_scenarios_stormwater.csv", sep="_") )
