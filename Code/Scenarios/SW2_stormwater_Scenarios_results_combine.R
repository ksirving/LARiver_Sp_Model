...
## time stats
ts <- list.files("output_data/scenarios/stormwater", pattern="time_stats_updated_hyd")
length(ts) ## 219
ts
j=1
...
for(j in 1: length(ts)) {
  ...
  time_stats <- read.csv(file=paste("output_data/scenarios/stormwater/", ts[j], sep=""))
...
write.csv(time_stats_all, "results/scenarios/stormwater/SW2_senarios_all_species_time_stats_updated_hyd.csv")
