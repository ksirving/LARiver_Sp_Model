...
min_limit <- read.csv("/Users/katieirving/Documents/git/flow_eco_mech/flow_recs/min_flow_limit_node_position.csv")
head(min_limit)
...
for(n in 1: length(h)) {
  ...
  hydraul <- NodeData %>%
    rename(Q = Flow) %>%
    mutate(node = NodeName, Scenario = 0)
  ...
  for(s in 1:length(scenarios)) {
    ...
    for(p in 1:length(positions)) {
      ...
      # min_limit_pos <- min_limit %>%
      #   select(Node, paste(positions[p])) %>%
      #   filter(Node == NodeName)
      # ?pnorm
      # min_limit_predict <- expression(pnorm(min_depth_limitx, mean=mean(depth_cm),
      #                               sd=sd(depth_cm), lower.tail = FALSE)) 
      # 
      # min_depth_limitx <- 3
      # 
      # new_data <- new_data %>%
      #   mutate(MeanProbabilityMinLimit = eval(min_limit_predict)) 
      # 
      # 
      head(new_data)
      ## define position
      ...
      prob_stats <- new_datax %>%
        ...
        # summarise(MaxProbability = max(Probability)) %>%
  ...
  write.csv(prob_statsx, paste("results/scenarios/probs/A1a",NodeName, "all_species_probs_baseline_probs.csv", sep="_"))
