...
for(n in 1: length(h)) {
  ...
  if(length(hydraul) == 14) {
    hyd_dep <- hydraul %>%
      ...
      select(Q, DateTime, Scenario, node,  depth_cm_MC:date_num)
  } else {
    hyd_dep <- hydraul %>%
      ...
      select(-contains("ft")) %>%
      mutate(date_num = seq(1,length(DateTime), 1)) %>%
      select(Q, DateTime, Scenario, node,  depth_cm_LOB:date_num)
  ...
  write.csv(prob_statsx, paste("results/scenarios/probs/A1a",NodeName, "all_species_probs_baseline_scenarios_probs.csv", sep="_"))
...
for(n in 1: length(h)) {
  ...
  write.csv(prob_statsx, paste("results/scenarios/probs/A1b",NodeName, "all_species_thresholds_baseline_scenarios_probs.csv", sep="_"))
