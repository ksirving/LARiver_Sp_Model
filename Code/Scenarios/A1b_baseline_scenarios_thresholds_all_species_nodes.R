...
dir <- "/Users/katieirving/Documents/git/flow_eco_mech/input_data/HecRas/"

h <- list.files(pattern="predictions")
length(h) ## 18
h
h <- h[-c(10,14)] ## remove LA1/ LA2
...
for(n in 1: length(h)) {
      ...
      for (l in 1:length(thresholds)) {
        ...
        prob_stats <- thresholds_datax %>%
          ...
          summarise(MeanProbability = mean(MeanProbabilityx), SDProbability = sd(MeanProbabilityx),
                    MaxProbability = max(MeanProbability)) %>%
  ...
  write.csv(prob_statsx, paste("results/scenarios/probs/A1b",NodeName, "all_species_thresholds_baseline_probs.csv", sep="_"))
