...
## time stats
ts <- list.files("ignore/Probs/", pattern="time_stats_updated_hyd")
length(ts) ## 219
ts
ts <- Filter(function(x) grepl("Cladophora", x), ts)
...
for(j in 1: length(ts)) {
  ...
  time_stats <- read.csv(file=paste("ignore/Probs/", ts[j], sep=""))
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Cladophora", Life_Stage = "N/A", Hydraulic = "Shear Stress", Node = paste(stringx[2])) %>%
    distinct()
...
time_stats_seas <- time_stats_seas %>%
  mutate(TimePercentage = ifelse(is.na(TimePercentage), 0, TimePercentage)) %>%
...
write.csv(time_stats_all, "results/Cladophera_Shear_Stress_time_stats_updated_hyd.csv")

# Days per month ----------------------------------------------------------


### days per month
td <- list.files("ignore/Probs/", pattern="total_days_long_updated_hyd")
length(td) ## 153

td <- Filter(function(x) grepl("Cladophora", x), td)
...
for(j in 1: length(td)) {
  ...
  total_days <- read.csv(file=paste("ignore/Probs/", td[j], sep=""))
...
total_days_seas <- total_days_seas %>%
  mutate(DaysPerMonth = ifelse(is.na(DaysPerMonth), 0, DaysPerMonth)) %>%
  mutate(Suitability_Class = NA)
...
write.csv(total_days_all,"results/Clad_Shear_Stress_total_days_updated_hyd.csv")
