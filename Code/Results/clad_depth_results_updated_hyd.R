...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Cladophora", Life_Stage = "N/A", Hydraulic = "Depth", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Clad_Depth_time_stats_updated_hyd.csv")
...
write.csv(total_days_all,"results/Clad_Depth_total_days_updated_hyd.csv")
