...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Steelhead", Life_Stage = "Migration", Hydraulic = "Velocity", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Steelhead_Migration_Velocity_time_stats_updated_hyd_prolonged.csv")
...
write.csv(total_days_all,"results/Steelhead_Migration_Velocity_total_days_updated_hyd_prolonged.csv")
