...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Typha", Life_Stage = "Seedling", Hydraulic = "Depth", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Typha_Seedling_Depth_time_stats_updated_hyd.csv")
...
write.csv(total_days_all,"results/Typha_Seedling_Depth_total_days_updated_hyd.csv")
