...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Santa Ana Sucker", Life_Stage = "Adult", Hydraulic = "Depth", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/SAS_Depth_Adult_time_stats_updated_hyd.csv")
...
write.csv(total_days_all,"results/SAS_Depth_Adult_total_days_updated_hyd.csv")
