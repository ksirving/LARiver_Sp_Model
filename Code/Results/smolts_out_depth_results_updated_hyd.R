...
length(ts) ## 219
ts
ts <- Filter(function(x) grepl("Smolts", x), ts)
...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Steelhead", Life_Stage = "Smolt", Hydraulic = "Depth", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Steelhead_Smolt_Depth_time_stats_updated_hyd.csv")
...
td <- Filter(function(x) grepl("Smolts", x), td)
...
for(j in 1: length(td)) {
  ...
  total_days <- total_days %>% 
    ...
    # mutate(Species = "Steelhead", Life_Stage = "Smolts", Hydraulic = "Depth", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(total_days_all,"results/Steelhead_Smolt_Depth_total_days_updated_hyd.csv")
