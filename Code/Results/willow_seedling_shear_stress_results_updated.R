...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Willow", Life_Stage = "Seedling", Hydraulic = "Shear Stress", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Willow_Seedling_Shear_Stress_time_stats_updated_hyd.csv")
...
write.csv(total_days_all,"results/Willow_Seedling_Shear_Stress_total_days_updated_hyd.csv")

unique(total_days_all$Node)
