...
for(j in 1: length(ts)) {
  ...
  time_stats <- time_stats %>% 
    ...
    # mutate(Species = "Cladophora", Life_Stage = "N/A", Hydraulic = "Velocity", Node = paste(stringx[2])) %>%
    distinct()
...
write.csv(time_stats_all, "results/Cladophera_Velocity_time_stats_updated_hyd.csv")
...
write.csv(total_days_all,"results/Clad_Velocity_total_days_updated_hyd.csv")
