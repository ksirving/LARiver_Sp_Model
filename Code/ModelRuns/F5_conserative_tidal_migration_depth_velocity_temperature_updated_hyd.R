...
for(n in 1: length(h)) {
  ...
  if(length(hydraul) == 13) {
    ...
    hyd_dep <- hydraul %>%
      ...
      mutate(sp_w_LOB = abs(Stream.Power..lb.ft.s..LOB*4.44822)/0.3048,
             sp_w_MC = abs(Stream.Power..lb.ft.s..MC*4.44822)/0.3048,
             sp_w_ROB = abs(Stream.Power..lb.ft.s..ROB*4.44822)/0.3048) %>%
  ...
  ## save out
  save(all_data, file=paste("output_data/F5_", NodeName, "_Steelhead_depth_Migration_discharge_probs_2010_2017_TS_updated_hyd_prolonged.RData", sep=""))
...
for(n in 1: length(h)) {
  ...
  for(p in 1:length(positions)) {
    ...
    time_stats <- new_data %>%
      dplyr::group_by(water_year, season) %>%
      dplyr::mutate(Seasonal = sum(Q >= min_limit & value <= newx2a)/length(DateTime)*100) %>%
      distinct(water_year,  Seasonal) %>%
      mutate(position= paste(PositionName), Node = NodeName)
    
    Q_Calc[p,] <- paste("Q >= min_limit & velocity <= newx2a")
    time_statsx <- rbind(time_statsx, time_stats)
    
    ### count days per month
    new_datax <- new_datax %>% 
      group_by(month, day, water_year, ID = data.table::rleid(Q >= min_limit & value <= newx2a)) %>%
      mutate(threshold = if_else(Q >= min_limit & value <= newx2a,  row_number(), 0L)) %>%
      mutate(position= paste(PositionName)) 
...
# 
#   ## save out
#   save(all_data, file=paste("output_data/F5_", NodeName, "_Steelhead_depth_Smolts_discharge_probs_2010_2017_TS_updated_hyd_prolonged.RData", sep=""))
# 
#   # format probability time series ------------------------------------------
# 
#   ### define dataframes for 2nd loop
...
# 
#   write.csv(limits, paste("output_data/F5_",NodeName,"_Steelhead_Smolts_depth_Q_limits_updated_hyd_prolonged.csv", sep=""))
# 
# 
# 
# 
...
#     mutate(Species ="Steelhead", Life_Stage = "Smolts", Hydraulic = "Depth", Node = NodeName)
# 
#   write.csv(melt_time, paste("output_data/F5_", NodeName, "_Steelhead_Smolts_depth_time_stats_updated_hyd_prolonged.csv", sep=""))
# 
#   ### days per month
#   days_data <- select(days_data,c(Q, month, water_year, month_year, year, day, ID, threshold, position, season, node))
# 
...
# 
#   ## save df
#   write.csv(melt_days, paste("output_data/F5_", NodeName, "_Steelhead_Smolts_depth_total_days_long_updated_prolonged_hyd.csv", sep="") )
# 
# } ## end 1st loop
# 
# # 
